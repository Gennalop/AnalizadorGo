...
def create_buttons_panel(self, r, c):
    ...
    self.down_icon = PhotoImage(file="gui/images/down.png")
    self.up_icon = PhotoImage(file="gui/images/up.png")

    tk.Button(actions_frame,  image=self.up_icon, bg=style.BUTTONS_T2_BG,  width=30, height=20, command=self.open_file).grid(row=0, column=0)
    #tk.Button(actions_frame,  image=self.down_icon, bg=style.BUTTONS_T2_BG,  width=30, height=20, command=self.open_file).grid(row=0, column=1, padx=4)
    tk.Button(actions_frame, text="Limpiar", bg=style.BUTTONS_T2_BG, command=self.clean_panels).grid(row=0, column=2, padx=4)
    self.boton_toggle_live = tk.Button(actions_frame, text="Live: OFF", bg=style.BUTTON_OFF_BG, command=self.toggle_live)
    self.boton_toggle_live.grid(row=0, column=4, padx=4)
    tk.Button(actions_frame, text="Ejecutar todo", bg=style.BUTTONS_T1_BG, command=self.analizar_todo).grid(row=0, column=5)
